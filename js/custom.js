...
var CarDealer = function(){
	...
	var MagnificPopup = function(){
		...
		jQuery('.popupgalerie').magnificPopup({
	...
	var priceslider = function(){
		...
		if($(".price-slide").length > 0 ) {
			var $sliderRange = $("#slider-range"),
				$sliderRangeMin = $('[name="prix-min"]', $sliderRange.parent()),
				$sliderRangeMax = $('[name="prix-max"]', $sliderRange.parent()),
				$sliderRangeText = $('.price-text', $sliderRange.parent());
			$sliderRange.slider({
				range: true,
				min: 100,
				max: 1000,
				values: [$sliderRangeMin.val(), $sliderRangeMax.val()],
				step: 50,
				slide: function(event, ui) {
					var min = ui.values[0],
						max = ui.values[1];
					$sliderRangeMin.val( min );
					$sliderRangeMax.val( max );
					$sliderRangeText.text( min + "€ - " + max + "€");
				}
			});
		}
		if($(".kms-slide").length > 0 ) {
			var $sliderRange2 = $("#slider-range2"),
				$sliderRange2Value = $('[name="kilometrage"]', $sliderRange2.parent()),
				$sliderRange2Text = $('.price-text', $sliderRange2.parent());
			$sliderRange2.slider({
				range: 'min',
				min: 1,
				max: 200000,
				value: $sliderRange2Value.val(),
				slide: function(event, ui) {
					var value = ui.value;
					$sliderRange2Value.val( value );
					$sliderRange2Text.text( "max " + value + "Km");
				}
			});
		}
	}
